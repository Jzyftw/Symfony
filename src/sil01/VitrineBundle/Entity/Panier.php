...
class Panier {
    ...
    // (vérifier si l'article n'y est pas déjà)
        if(array_key_exists($articleId, $this->contenu)){
            $this->contenu[$articleId]['quantite'] = $this->contenu[$articleId]['quantite'] + $qte ; 
            $this->contenu[$articleId]['prix'] = $this->contenu[$articleId]['quantite'] * $this->contenu[$articleId]['prix'];
        }
